...
//		return format(vo, run, true);
//	}
//
//	@NotNull
//	public static <V> List<V> format(List<V> vo, @Nullable Consumer3<V, Integer, Map<FieldType, Map<?, ?>>> run, boolean annotationsArePreceded) {
//		try {
//			if (CollUtil.isEmpty(vo)) return Collections.emptyList();
//
//			if (init(vo.get(0).getClass())) return vo;
//
//			forEach(vo.iterator(), initData(vo), annotationsArePreceded ? ConsumerBuilder.<V>builder().add(FormatUtil::reflectionField).add(run).build() : ConsumerBuilder.<V>builder().add(run).add(FormatUtil::reflectionField).build());
//			return vo;
//		} finally {
//			clear();
//		}
...
//					break;
//				case TIMESTAMP:
//					Opt.ofNullable(value).ifPresent(fieldValue -> ReflectUtil.setFieldValue(vo, entry.getKey(), getTimestamp(fieldValue)));
//					break;
//				default:
//					break;
//			}
...
//			return Opt.ofTry(() -> DateUtil.parse((String) value)).map(DateTime::toLocalDateTime).map(LocalDateTimeUtil::toEpochMilli).exceptionOrElse(null);
//		} else if (value instanceof Date) {
//			return ((Date) value).getTime();
//		} else if (value instanceof Calendar) {
//			return ((Calendar) value).getTimeInMillis();
//		} else if (value instanceof Long) {
//			return Opt.of((Long) value).filter(v -> v >= 0 && v <= 253402271999999L).orElse(null);
//		} else {
//			return defaultValue;
//		}
//	}
