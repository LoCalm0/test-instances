//package indi.LoCalm.format.version4.util;
//
...
//				setAnnotationValue.put(fieldName, Opt.ofNullable((DeptFormat) annotation).map(deptFormat -> MapUtil.builder(new HashMap<String, Object>()).put(SET_FIELD_NAME, deptFormat.setFieldName().getField()).put(GET_SEPARATOR, Opt.ofBlankAble(deptFormat.getSeparator()).orElse(null)).put(SET_SEPARATOR, Opt.ofBlankAble(deptFormat.setSeparator()).orElse(Opt.ofBlankAble(deptFormat.getSeparator()).orElse(null))).build()).orElse(Collections.emptyMap()));
//				break;
//			case "TenantFormat":
//				setIdentity.put(fieldName, FieldType.TENANT);
//				setAnnotationValue.put(fieldName, ((TenantFormat) annotation).setFieldName().getField());
//				break;
//			case "DictFormat":
//				setIdentity.put(fieldName, FieldType.DICT);
//				setAnnotationValue.put(fieldName, ((DictFormat) annotation).code().name().toLowerCase());
//				break;
//			case "DictBizFormat":
//				setIdentity.put(fieldName, FieldType.DICT_BIZ);
//				setAnnotationValue.put(fieldName, ((DictBizFormat) annotation).code().name().toLowerCase());
//				break;
//			case "DateTimeFormat":
//				setIdentity.put(fieldName, FieldType.DATE_TIME);
//				setAnnotationValue.put(fieldName, ((DateTimeFormat) annotation).pattern());
//				break;
//			case "WeekFormat":
...
//	private static <V> void reflectionField(V vo, int index, @NotNull Map<FieldType, Map<?, ?>> data) {
//		Map<String, String> setGetFieldName = SET_GET_FIELD_NAME.get();
//		Map<String, ? super Object> setAnnotationValue = SET_ANNOTATION_VALUE.get();
//		Map<Long, User> userMap = (Map<Long, User>) data.get(FieldType.USER);
//		Map<Long, Dept> deptMap = (Map<Long, Dept>) data.get(FieldType.DEPT);
//		Map<String, Tenant> tenantMap = (Map<String, Tenant>) data.get(FieldType.TENANT);
...
//					break;
//				case TENANT:
//					Opt.ofNullable(tenantMap.get(Convert.toStr(value))).ifPresent(tenant -> ReflectUtil.setFieldValue(vo, entry.getKey(), ((Function<Tenant, ?>) setAnnotationValue.get(entry.getKey())).apply(tenant)));
//					break;
//				case DICT:
...
//					break;
//				case DATE_TIME:
//					Opt.ofNullable(Convert.toLong(value)).ifPresent(fieldValue -> ReflectUtil.setFieldValue(vo, entry.getKey(), DateUtil.date(fieldValue).toString(Convert.toStr(setAnnotationValue.get(entry.getKey())))));
//					break;
//				case WEEK:
//					Opt.ofNullable(Convert.toLong(value)).ifPresent(fieldValue -> ReflectUtil.setFieldValue(vo, entry.getKey(), weekOfYear(fieldValue)));
//					break;
//				default:
//					break;
//			}
...
//		Opt.ofNullable(fieldMap.get(FieldType.TENANT)).ifPresent(map -> data.put(FieldType.TENANT, getTenantMap(Opt.ofNullable(fieldMap.get(FieldType.TENANT)).map(Map::keySet).orElse(Collections.emptySet()), vo)));
...
//
//		Set<String> tenantGetField = Opt.ofNullable(fieldMap.get(FieldType.TENANT)).map(Map::keySet).orElse(Collections.emptySet());
//
...
//						break;
//					case TENANT:
//						tenantGetField.forEach(name -> tenantIds.add(Convert.toStr(ReflectUtil.getFieldValue(entity, name))));
//						break;
//					default:
//						break;
//				}
...
//		Opt.ofNullable(fieldMap.get(FieldType.TENANT)).ifPresent(e -> data.put(FieldType.TENANT, getEntityMapAfter(tenantIds, SysCache::getTenantByTenantIds, Tenant::getTenantId, "获取租户失败")));
//		Opt.ofNullable(fieldMap.get(FieldType.DICT)).ifPresent(map -> data.put(FieldType.DICT, getDictMap(map.keySet().toArray(new String[0]), DictCache::getList, Dict::getCode, Dict::getDictKey, Dict::getDictValue)));
//		Opt.ofNullable(fieldMap.get(FieldType.DICT_BIZ)).ifPresent(map -> data.put(FieldType.DICT_BIZ, getDictMap(map.keySet().toArray(new String[0]), DictBizCache::getList, DictBiz::getCode, DictBiz::getDictKey, DictBiz::getDictValue)));
//		return data;
//	}
...
//	}
//
//	public static <V> Map<String, Tenant> getTenantMap(Set<String> tenantGetField, V vo) {
//		if (CollUtil.isEmpty(tenantGetField)) return Collections.emptyMap();
//		if (tenantGetField.size() == 1) {
//			return Opt.ofNullable(Convert.toStr(ReflectUtil.getFieldValue(vo, tenantGetField.iterator().next()))).map(id -> Opt.ofNullable(SysCache.getTenant(id)).map(entity -> Collections.singletonMap(id, entity)).orElse(Collections.emptyMap())).orElse(Collections.emptyMap());
//		} else {
//			return getEntityMapAfter(tenantGetField.stream().map(name -> Convert.toStr(ReflectUtil.getFieldValue(vo, name))).collect(Collectors.toSet()), SysCache::getTenantByTenantIds, Tenant::getTenantId, "获取租户失败");
//		}
//	}
//
//	@NotNull
...
//						break;
//					case TENANT:
//						Opt.ofNullable(fieldMap.get(FieldType.TENANT)).ifPresentOrElse(map -> map.put(Convert.toStr(setGetFieldName.get(entry.getKey())), null), () -> fieldMap.put(FieldType.TENANT, MapUtil.builder(new HashMap<String, String>()).put(Convert.toStr(setGetFieldName.get(entry.getKey())), null).build()));
//						break;
//					case DICT:
...
//	public static long[] getStartAndEndTimestamp(LocalDateTime localDateTime, @NotNull TemporalUnit unit) {
//		return getStartAndEndTimestamp(localDateTime, unit, null);
//	}
//
//	@Contract("null,_,_ -> param3")
//	public static long[] getStartAndEndTimestamp(LocalDateTime localDateTime, @NotNull TemporalUnit unit, long[] defaultValue) {
//		return getStartAndEndTimestamp(localDateTime, 1, unit, defaultValue);
//	}
//
//	@Contract("null,_,_,_ -> param4")
//	public static long[] getStartAndEndTimestamp(LocalDateTime localDateTime, long amountToAdd, @NotNull TemporalUnit unit, long[] defaultValue) {
...
//
//	@Contract("null,_ -> null")
//	public static long[] getStartAndEndTimestamp(LocalDate localDate, @NotNull TemporalUnit unit) {
//		return getStartAndEndTimestamp(localDate, unit, null);
//	}
//
//	@Contract("null,_,_ -> param3")
//	public static long[] getStartAndEndTimestamp(LocalDate localDate, @NotNull TemporalUnit unit, long[] defaultValue) {
//		return getStartAndEndTimestamp(localDate, 1, unit, defaultValue);
//	}
//
//	@Contract("null,_,_,_ -> param4")
//	public static long[] getStartAndEndTimestamp(LocalDate localDate, long amountToAdd, @NotNull TemporalUnit unit, long[] defaultValue) {
